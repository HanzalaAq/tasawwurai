...
export function VoiceInput({ onSend, disabled, onFinalTranscript }: Props) {
  ...
  if (!isSupported) {
    return (
      <div className="voice-input-bar">
        <div className="flex items-center gap-3 rounded-2xl border border-amber-500/30 bg-gray-900/95 px-5 py-3 shadow-2xl backdrop-blur-xl">
  ...
  return (
    <div className="voice-input-bar">
      ...
      {errorMsg && (
        <div className="mb-2 flex items-center gap-2 rounded-xl border border-red-500/30 bg-red-950/80 px-4 py-2 shadow-lg backdrop-blur-md animate-fade-in">
      ...
      {/* Main floating bar */}
      <div
        className={`voice-input-container flex items-center gap-4 rounded-full border px-2 py-2 shadow-2xl backdrop-blur-xl transition-all duration-500 ${
          isListening
            ? "border-red-500/40 bg-gray-900/95 shadow-red-500/10 voice-input-active"
            : "border-gray-700/50 bg-gray-900/95"
        }`}
      >
        {/* Left: Waveform + interim text */}
        <div className="flex flex-1 items-center justify-end min-w-0 px-3">
          {isListening && interimTranscript ? (
            <p className="voice-input-interim truncate text-sm text-blue-200/90">
              {interimTranscript}
              <span className="interim-cursor ml-0.5 inline-block h-4 w-[2px] translate-y-0.5 bg-blue-400" />
            </p>
          ) : isListening ? (
            <div className="flex items-center gap-[3px] h-8">
              {Array.from({ length: 20 }).map((_, i) => (
                <span
                  key={i}
                  className="waveform-bar inline-block w-[2.5px] rounded-full bg-blue-400/60"
        ...
        <button
          ...
          className={`voice-mic-button relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full transition-all duration-300 focus:outline-none ${
            isListening
              ? "bg-red-500 shadow-lg shadow-red-500/50 hover:bg-red-400 scale-110"
              : "bg-gradient-to-br from-blue-500 to-blue-700 shadow-lg shadow-blue-500/30 hover:from-blue-400 hover:to-blue-600 hover:scale-105"
        ...
        >
          ...
          {isListening && (
            <>
              <span className="mic-ping absolute inset-0 rounded-full bg-red-400/30" />
              <span className="mic-ripple absolute inset-[-4px] rounded-full border-2 border-red-400/40" />
              <span
                className="mic-ripple absolute inset-[-8px] rounded-full border border-red-400/20"
        ...
        <div className="flex flex-1 items-center min-w-0 px-3">
          {isListening ? (
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="processing-dot h-1.5 w-1.5 rounded-full bg-blue-400" style={{ animationDelay: "150ms" }} />
              <span className="processing-dot h-1.5 w-1.5 rounded-full bg-blue-400" style={{ animationDelay: "300ms" }} />
              <span className="ml-1 text-xs text-blue-400">Analyzing</span>
            </div>
          ) : (
